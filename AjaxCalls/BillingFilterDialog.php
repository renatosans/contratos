...
// Dialogo com os filtros do relatório de faturamento
...
<form name="fDados" >
    ...
    <label class="left" style="width:45%; text-align: left;">Mês do Faturamento<br/>
        <select name="billingMonth" style="width:95%;height:30px;" ><?php $calendar = new Calendar(); echo $calendar->GetMonthOptions(0); ?></select>
    </label>
    <label class="left" style="width:45%; text-align: left;">Ano do Faturamento<br/>
        <input type="text" name="billingYear" style="width:95%;height:30px;" value="<?php echo date('Y'); ?>" ></input>
    </label>
...
<script type="text/javascript" >
    ...
    function OkButtonClicked() {
        ...
        var billingMonth = $("select[name=billingMonth]").val();
        var billingYear = $("input[name=billingYear]").val();
        var salesPerson = $("select[name=salesPerson]").val();
        var parameters = '?businessPartnerCode=' + businessPartnerCode + '&model=' + model + '&equipmentCode=' + equipmentCode + '&billingMonth=' + billingMonth + '&billingYear=' + billingYear + '&salesPerson=' + salesPerson;
